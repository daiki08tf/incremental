...
(function (Game) {
  ...
  // 優先順位: 1.次に解放できる建物 → 2.次の建物マイルストーン
  //           → 3.脱出条件 (4.ジャンプ可能時は予想KPを最終目標として表示)
  Game.getNextGoal = function (state) {
    if (Game.canPrestige(state)) {
      return {
        kind: 'jump',
        text: '脱出船が発進できる — 予想知識 +' + Game.calcKnowledgeGain(state),
      };
    }
    ...
    if (milestoneGoal) return milestoneGoal;

    var req = Game.ESCAPE_REQUIREMENTS;
    return {
      kind: 'escape',
      text: '脱出船 — 部品 ' + Game.formatNumber(state.resources.components) + '/' + Game.formatNumber(req.components)
        + ' ・ 金属 ' + Game.formatNumber(state.resources.metal) + '/' + Game.formatNumber(req.metal),
    };
  };
